...
use crate::constants::LESS_WHITE;
use crate::data::AlbumSizing;
use egui::{Label, RichText, ScrollArea, Sense, TextStyle};
    ...
    pub fn show_tab_albums(
        ...
        match self.settings.album_sizing {
            AlbumSizing::Pixel => {
                ...
                // How many rows?
                let rows =
                    (self.collection.count_album.inner() as f32 / album_width).ceil() as usize;
                            ...
                            match iter.next() {
                                Some(key) => {
                                    ui.vertical(|ui| {
                                        ...
                                        let label = Label::new(
                                            RichText::new(
                                                album.title.head_dot(ALBUM_TITLE_LIMIT).as_str(),
                                            )
                                            .color(LESS_WHITE),
                                        );
                                        let resp = ui
                                            .add_sized([pixel, 0.0], label)
                                            .on_hover_text(&*album.title);

                                        // Artist name.
                                        let (artist, _) = &self.collection.artist_from_album(key);
                                        let artist_name =
                                            Label::new(artist.name.head_dot(label_width).as_str())
                                                .sense(Sense::click());
                                        // We don't use `crate::artist_label!()` here
                                        // because we need a custom `ui.add_sized()`
                                        let resp = ui
                                            .add_sized([pixel, 0.0], artist_name)
                                            .on_hover_text(&*artist.name);
                                        if resp.clicked() {
                                            if self.modifiers.command {
                                                self.playlist_add_screen =
                                                    Some(shukusai::collection::KeyEnum::Artist(
                                                        artist.key,
                                                    ));
                                            } else {
