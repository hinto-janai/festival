...
impl Watch {
    ...
    fn clean() {
        ...
        if let Err(e) = Pause::mkdir() {
            error!("Watch - Could not create signal folder {e}");
        }

        // Clean files.
        if let Err(e) = Toggle::rm() {
            error!("Watch - Toggle: {e}");
        }
        if let Err(e) = Pause::rm() {
            error!("Watch - Pause: {e}");
        }
        if let Err(e) = Play::rm() {
            error!("Watch - Play: {e}");
        }
        if let Err(e) = Next::rm() {
            error!("Watch - Next: {e}");
        }
        if let Err(e) = Previous::rm() {
            error!("Watch - Previous: {e}");
        }
        if let Err(e) = Stop::rm() {
            error!("Watch - Stop: {e}");
        }
        if let Err(e) = Shuffle::rm() {
            error!("Watch - Shuffle: {e}");
        }
        if let Err(e) = RepeatSong::rm() {
            error!("Watch - RepeatSong: {e}");
        }
        if let Err(e) = RepeatQueue::rm() {
            error!("Watch - RepeatQueue: {e}");
        }
        if let Err(e) = RepeatOff::rm() {
            error!("Watch - RepeatOff: {e}");
        }

        // Content files.
        if let Err(e) = Volume::rm() {
            error!("Watch - Volume: {e}");
        }
        if let Err(e) = Seek::rm() {
            error!("Watch - Seek: {e}");
        }
        if let Err(e) = SeekForward::rm() {
            error!("Watch - SeekForward: {e}");
        }
        if let Err(e) = SeekBackward::rm() {
            error!("Watch - SeekBackward: {e}");
        }
        if let Err(e) = Index::rm() {
            error!("Watch - Index: {e}");
        }
        if let Err(e) = Clear::rm() {
            error!("Watch - Clear: {e}");
        }
        if let Err(e) = Skip::rm() {
            error!("Watch - Skip: {e}");
        }
        if let Err(e) = Back::rm() {
            error!("Watch - Back: {e}");
        }
    ...
    fn main(self) {
        ...
        loop {
            ...
            if Shuffle::exists().is_ok() {
                self.send(WatchToKernel::Shuffle);
            }
            if RepeatSong::exists().is_ok() {
                self.send(WatchToKernel::RepeatSong);
            }
            if RepeatQueue::exists().is_ok() {
                self.send(WatchToKernel::RepeatQueue);
            }
            if RepeatOff::exists().is_ok() {
                self.send(WatchToKernel::RepeatOff);
            }

            // Content signals.
            if let Ok(v) = Volume::from_file() {
                self.send(WatchToKernel::Volume(v.0));
            }
            if let Ok(s) = Skip::from_file() {
                self.send(WatchToKernel::Skip(s.0));
            }
            if let Ok(s) = Index::from_file() {
                self.send(WatchToKernel::Index(s.0.saturating_sub(1)));
            }
            if let Ok(s) = Clear::from_file() {
                self.send(WatchToKernel::Clear(s.0));
            }
            if let Ok(s) = Seek::from_file() {
                self.send(WatchToKernel::Seek(s.0));
            }
            if let Ok(s) = SeekForward::from_file() {
                self.send(WatchToKernel::SeekForward(s.0));
            }
            if let Ok(s) = SeekBackward::from_file() {
                self.send(WatchToKernel::SeekBackward(s.0));
            }
            if let Ok(s) = Back::from_file() {
                self.send(WatchToKernel::Back(s.0));
            }
...
mod tests {
    ...
    fn signals() {
        ...
        assert_eq!(
            from_watch.recv_timeout(T).unwrap(),
            WatchToKernel::RepeatSong
        );
        sleep!(S);
        ...
        for i in [0, 50, 100, 101, u8::MAX] {
            ...
            assert_eq!(
                from_watch.recv_timeout(T).unwrap(),
                WatchToKernel::Volume(crate::audio::Volume::new(i))
            );
            sleep!(S);
        }
        ...
        for i in [0, 1, 5, usize::MAX] {
            ...
            assert_eq!(
                from_watch.recv_timeout(T).unwrap(),
                WatchToKernel::Index(i.saturating_sub(1))
            );
            sleep!(S);
        }
