...
//---------------------------------------------------------------------------------------------------- Collection
disk::bincode2!(Collection, disk::Dir::Data, FESTIVAL, formatcp!("{FRONTEND_SUB_DIR}/{STATE_SUB_DIR}"), "collection", HEADER, 1);
...
impl Into<crate::collection::Collection> for Collection {
	fn into(self) -> crate::collection::Collection {
		...
		let artists: crate::collection::Artists = artists.0
			.into_iter()
			.enumerate()
			.map(|(k, v)| {
				let v: crate::collection::Artist = v.into();
				v.key = ArtistKey::from(k);
				v
			})
			.collect()
			.into();
		let albums: crate::collection::Albums = albums.0
			.into_iter()
			.enumerate()
			.map(|(k, v)| {
				let v: crate::collection::Album = v.into();
				v.key = AlbumKey::from(k);
				v
			})
			.collect()
			.into();
		let songs: crate::collection::Songs = songs.0
			.into_iter()
			.enumerate()
			.map(|(k, v)| {
				let v: crate::collection::Song = v.into();
				v.key = SongKey::from(k);
				v
			})
			.collect()
			.into();
