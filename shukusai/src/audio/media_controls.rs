//---------------------------------------------------------------------------------------------------- Use
use crate::constants::{FESTIVAL, FESTIVAL_DBUS};
...
pub(super) fn init_media_controls(
    to_audio: Sender<souvlaki::MediaControlEvent>,
) -> Result<souvlaki::MediaControls, anyhow::Error> {
    #[cfg(windows)]
    let hwnd = {
        ...
        pub(super) fn new() -> Result<DummyWindow, anyhow::Error> {
            ...
            let handle_result = unsafe {
                let instance = GetModuleHandleW(None)
                    .map_err(|e| (anyhow!("Getting module handle failed: {e}")))?;
