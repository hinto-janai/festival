...
input.data <- rbind(
  ...
  data.frame(x = x.musicbee, y = y.musicbee, series = "MusicBee (118)"),
  data.frame(x = x.gnome,    y = y.gnome,    series = "GNOME Music (33)"),
  data.frame(x = x.lollypop, y = y.lollypop, series = "Lollypop (0)"),
  data.frame(x = x.itunes,   y = y.itunes,   series = "iTunes (35)")
)
