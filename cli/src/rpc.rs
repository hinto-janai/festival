...
pub fn request(config: Config, rpc: Rpc) -> ! {
	...
	match rpc {
		...
		AddQueueKeyArtist(x)  => req_resp!(x, rpc::resp::Status),
		AddQueueKeyAlbum(x)   => req_resp!(x, rpc::resp::Status),
		AddQueueKeySong(x)    => req_resp!(x, rpc::resp::Status),
		AddQueueMapArtist(x)  => req_resp!(x, rpc::resp::Status),
		AddQueueMapAlbum(x)   => req_resp!(x, rpc::resp::Status),
		AddQueueMapSong(x)    => req_resp!(x, rpc::resp::Status),
		AddQueueRandArtist(x) => req_resp!(x, rpc::resp::AddQueueRandArtist),
		AddQueueRandAlbum(x)  => req_resp!(x, rpc::resp::AddQueueRandAlbum),
		AddQueueRandSong(x)   => req_resp!(x, rpc::resp::AddQueueRandSong),
		SetQueueIndex(x)      => req_resp!(x, rpc::resp::SetQueueIndex),
		RemoveQueueRange(x)   => req_resp!(x, rpc::resp::RemoveQueueRange),
	}
}
