...
pub fn request(config: Config, rpc: Rpc) -> ! {
	...
	match rpc {
		...
		PlaylistRemoveSong(x) => req_resp!(x, rpc::resp::PlaylistRemoveSong),
		PlaylistAddArtist(x)  => req_resp!(x, rpc::resp::PlaylistAddArtist),
		PlaylistAddAlbum(x)   => req_resp!(x, rpc::resp::PlaylistAddAlbum),
		PlaylistAddSong(x)    => req_resp!(x, rpc::resp::PlaylistAddSong),
