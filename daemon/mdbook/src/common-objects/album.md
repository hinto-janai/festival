...
The [keys](/common-objects/key.md) inside `songs` is sorted by `Track + Disc order`.

| Field      | Type                                      | Description |
|------------|-------------------------------------------|-------------|
| title      | string                                    | The title of this `Album`
| key        | `Album` key (unsigned integer)            | The `Album` key associated with this `Album`
| artist     | [`Artist`](/common-objects/artist.md) key (unsigned integer)           | The `Artist` key of the `Artist` that owns this `Album`
...
| song_count | unsigned integer                          | How many [`Song`](/common-objects/song.md)'s are in this `Album`
